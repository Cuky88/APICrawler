...
if clusterAlgo==1:
    ...
    i = 0
    for e in erg:
        quality = Comparator.compare(e)
        qualityStorage.append([quality, [params[i], params[i]]])
        i = i+1

    quality = qualityStorage[0]

    for i in range(0, len(qualityStorage)):
        print("Parameter (#Cluster (k) => " + str(qualityStorage[i][1][0]) + ", #Improvementiterations (n) => " + str(
            qualityStorage[i][1][1]) + " has quality: Rand statistics " + str(
            qualityStorage[i][0][0]) + " and Jaccard Coeffcient " + str(qualityStorage[i][0][1]))
